...
- (instancetype)initWithNativeAd:(ADGNativeAd *)nativeAd {
    ...
    if (self) {
        ...
        if (nativeAd.video && nativeAd.video.isValid) {
            mainImageView = [[UIImageView alloc] initWithFrame:CGRectMake(4, 46, 292, 170)];
            self.adgVideoView = [[ADGVideoView alloc] initWithFrame:CGRectMake(0, 0 , 292 , 170)];
            self.adgVideoView.video = nativeAd.video;
            [mainImageView addSubview:self.adgVideoView];
        }
        else if (nativeAd.mainImage.url.length > 0) {
            mainImageView = [[UIImageView alloc] initWithFrame:CGRectMake(4, 64, 292, 156)];
        ...
        [self addSubview:adTextLabel];
        [self addSubview:mainImageView];
        [self addSubview:descriptionLbl];
        [self addSubview:sponsoredLbl];
